...
echo "https://github.com/gentilkiwi/mimikatz/releases/download/2.2.0-20220919/mimikatz_trunk.zip" | sudo tee -a download.txt
...
# Preparo il file per le altre (RFI, SSRF, ...) injection
cd $folderProjectEngine
...
tmux send-keys -t PT:8.4 "# SSTI automation (save burp file with name: burp.req)" Enter
tmux send-keys -t PT:8.4 "ffuf -request burp.req -request-proto http -w $folderProjectWebAuthN/out-injection-list.txt -fl 120"
tmux send-keys -t PT:8.5 "# SSTI automation (GET)" Enter
...
cd $folderProjectWebAuthN
# Layout
tmux new-window -t PT:11 -n 'SQLi'
tmux split-window -v -t PT:11.0
tmux select-pane -t "11.0"
tmux split-window -h -t "11.0"
tmux split-window -h -t "11.0"
